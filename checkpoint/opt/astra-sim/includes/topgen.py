...
class Node:
  ...
  def __str__ (this):
    ids = [str(node.sid()) + '_' + str (node.id ()) for node in this.ports ()]
    typestr = ''
...
class GenericLink (Node):
  ...
  def tx (this, ntx = None):
    if ntx is not None:
      this._tx = ntx
    return this._tx

  def rx (this, nrx = None):
    if nrx is not None:
      this._rx = nrx
    return this._rx
...
class NVLink3 (GenericLink):
  def __init__ (this, src, dst = None):
    super ().__init__ (src, dst)
    this._sid = 'NVLink3'
    # add specs
    # using NVLink Ampere 3rd gen as bandwidth and latency
    # https://www.nvidia.com/en-us/data-center/nvlink/
    # https://community.fs.com/article/an-overview-of-nvidia-nvlink.html
    # 1.3pJ per bit
    # 4 Signal Pairs at 50Gbps each
    this.bw (400e9) # 50GBps
    this.latency (7500e-9) # Based on P2P Latency
    this.idle (0.0)
    this.peak (5.5)
    this.tx (8.25)
    this.rx (8.26)

class NVLink4 (GenericLink):
  def __init__ (this, src, dst = None):
    super ().__init__ (src, dst)
    this._sid = 'NVLink4'
    # 4th Generation NVLink (https://resources.nvidia.com/en-us-hopper-architecture/nvidia-h100-tensor-c)
    # uses 2 high speed signal pairs in each direction (25 Gbps each direction)
    # H100 used 18 4th generation NVLinks with total 900GB/s (both directions)
    # Connectivity (5 4 4 5) (https://developer.nvidia.com/blog/?p=53977)
    this.bw (400e9) # 50GBps
    this.latency (6500e-9) # Based on P2P Latency
    this.idle (0.0)
    this.peak (5.5)
    this.tx (7.61)
    this.rx (7.62)
...
class PCIe4 (GenericLink):
  def __init__ (this, src, dst = None, lanes = 16):
    super ().__init__ (src, dst)
    this._sid = 'PCIe4'
    this.bw (32e9 * lanes) # 4GBps per lane
    this.latency (5000e-9) # assume 5us
    this.idle (0.0)
    this.peak (0.0) # remove consumption from link (apply to GPU and Card)

class PCIe5 (GenericLink):
  def __init__ (this, src, dst = None, lanes = 16):
    super ().__init__ (src, dst)
    this._sid = 'PCIe5'
    this.bw (64e9 * lanes) # 8GBps per lane
    this.latency (5000e-9) # assume 5us
    this.idle (0.0)
    this.peak (0.0) # remove consumption from link (apply to GPU and Card)
...
class NVLinkSwitch (GenericSwitch):
  def __init__ (this):
    ...
    this._sid = 'NVLinkSwitch'

class NVLinkSwitch3 (GenericSwitch):
  def __init__ (this):
    super ().__init__ (64, [
      NVLink4, NVLink4, NVLink4, NVLink4, NVLink4, NVLink4, NVLink4, NVLink4,
      NVLink4, NVLink4, NVLink4, NVLink4, NVLink4, NVLink4, NVLink4, NVLink4,
      NVLink4, NVLink4, NVLink4, NVLink4, NVLink4, NVLink4, NVLink4, NVLink4,
      NVLink4, NVLink4, NVLink4, NVLink4, NVLink4, NVLink4, NVLink4, NVLink4,
      NVLink4, NVLink4, NVLink4, NVLink4, NVLink4, NVLink4, NVLink4, NVLink4,
    ...
    this._sid = 'NVLinkSwitch3'

class NetworkCardSwitch (GenericSwitch):
  def __init__ (this, PCIInterconnect, npcie, InfiniInterconnect, ninfiniband):
    types = [PCIInterconnect for _ in range (npcie)]
    types.extend ([InfiniInterconnect for _ in range (ninfiniband)])
...
class GenericCompute (GenericEndpoint):
  def __init__ (this, nports = 1, types = None):
    ...
    this._bw = 1e9
    this._l2 = 1e6
    this._sid = 'GenericCompute'
    this._idle_power = 0.0
    this._compute_power = 0.0
    this._memory_power = 0.0
    this._tdp = 0.0

  def idle_power (this, nidle_power = None):
    if nidle_power is not None:
      this._idle_power = nidle_power
    return this._idle_power

  def compute_power (this, ncompute_power = None):
    if ncompute_power is not None:
      this._compute_power = ncompute_power
    return this._compute_power

  def memory_power (this, nmemory_power = None):
    if nmemory_power is not None:
      this._memory_power = nmemory_power
    return this._memory_power

  def tdp (this, ntdp = None):
    if ntdp is not None:
      this._tdp = ntdp
    return this._tdp
...
class A100SXM (GenericCompute):
  def __init__ (this):
    ...
    this.bw (1250.0e9)
    this.perf (280.0e12) # change to float16 bandwidth
    this.idle (60.0)
    this.peak (400.0)
    this._sid = 'A100SXM'

class H200SXM (GenericCompute):
  def __init__ (this):
    # H100s have 18 NVLINK ports and 1 PCIe Port >> Infiniband Port w/ Adapter
    super ().__init__ (19, [
      NVLink4, NVLink4, NVLink4, NVLink4, NVLink4, NVLink4, NVLink4, NVLink4, NVLink4,
      NVLink4, NVLink4, NVLink4, NVLink4, NVLink4, NVLink4, NVLink4, NVLink4, NVLink4,
      PCIe5
    ])
    this.bw (3.35e12)
...
class DGXH200 (GenericEndpoint):
  def __init__ (this):
    ...
    # connect GPUs
    for i, h100 in enumerate (this._nodes):
      for j, nvswitch in enumerate (this._int_switches):
