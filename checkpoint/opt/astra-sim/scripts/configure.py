...
def parse_system_config (design):
  system = {
    # -- required arguments
    'req': {
      'all-reduce': [],
...
def create_project_dirs (target_design, target_workload, overwrite):
  ...
  return proj, inputs, outputs

def create_system_config (inputs, system):
  scfg = os.path.join (inputs, 'system_cfg.json')
  sjson = dict ()
  for arg in system['req'].keys ():
    sjson[str (arg) + '-implementation'] = [str (item) for item in system['req'][arg]]
  for arg in system['opt'].keys ():
    sjson[str (arg)] = system['opt'][arg]
  with open (scfg, 'w') as cfg:
    cfg.write (json.dumps (sjson, indent = 2))
  return scfg

def create_log_config (inputs): 
...
def create_memory_config (inputs):
...
def create_workload (filename, inputs, design, ncount, target_workload):
  ...
  if workload['format'] == 'text':
    ...
    if workload.get ('sample', False) == True:
      ...
      workload_location = os.path.join (
              ...
              'text_workloads')
    # get num passes
    npasses = workload.get ('num-passes', 1)
...
def generate_config_ns3 (filename, design, target_design, target_workload, binary, overwrite = True):
  ...
  # parse system config
  system = parse_system_config (design)
  ...
  proj, inputs, outputs = create_project_dirs (target_design, target_workload, overwrite)
  ...
  # make system
  scfg = create_system_config (inputs, system)

  # make logger
  lcfg = create_log_config (inputs)

  # make memory
  mcfg = create_memory_config (inputs)

  # make workload
  wdir, workload = create_workload (filename, inputs, design, ncount, target_workload)
