...
void Link::schedule_chunk_transmission(std::unique_ptr<Chunk> chunk) noexcept {
    ...
    // Jalil Morris data
    auto src = chunk->current_device ()->get_id ();
    auto dst = chunk->next_device ()->get_id ();
    ...
    std::cout << "[tracker] " << 
		 ...
		 chunk_size << "," <<
		 current_time << "," <<
		 current_time + communication_time << "," <<
		 current_time << "," <<
		 current_time + serialization_time << std::endl;
}
