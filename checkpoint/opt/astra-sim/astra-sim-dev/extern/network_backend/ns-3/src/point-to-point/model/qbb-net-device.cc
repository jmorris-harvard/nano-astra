...
namespace ns3 {
		...
		QbbNetDevice::TransmitComplete(void)
	{
		...
		this->Trace (1 /* DATA */, 2 /* PROPAGATE */, m_currentPkt);
	...
	void QbbNetDevice::Trace (uint64_t type, uint64_t loc, Ptr<Packet> p) {
	  ...
	  arguments[2] = node->GetId ();
	  arguments[3] = this->GetIfIndex ();
	  arguments[4] = p->GetSize ();
	  p->trace (arguments);	
	}
	// --- Jalil
		...
		QbbNetDevice::Resume(unsigned qIndex)
	{
        ...
        // --- Jalil
	void
		QbbNetDevice::ReceiveStart (Ptr<Packet> packet)
	{
		this->Trace (1 /* DATA */, 3 /* ARRIVE */, packet);
		m_macRxTrace (packet);
	}
	// --- Jalil
		...
		QbbNetDevice::Receive(Ptr<Packet> packet)
	{
		...
		this->Trace (1 /* DATA */, 4 /* RECEIVE */, packet);
		...
		QbbNetDevice::TransmitStart(Ptr<Packet> p)
	{
		...
		m_currentPkt = p;
		this->Trace (1 /* DATA */, 1 /* TRANSMIT */, p);
