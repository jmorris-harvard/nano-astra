...
uint32_t RdmaHw::GetNicIdxOfQp(Ptr<RdmaQueuePair> qp){
	...
	// --- Jalil ---
        /*
	for (auto it1 = m_rtTable.begin (); it1 != m_rtTable.end (); it1++) {
	  auto v = it1->second;
...
// ---- Jalil ----
void RdmaHw::AddQueuePair(uint32_t src, uint32_t dest, uint64_t tag, uint64_t size, uint16_t pg, Ipv4Address sip, Ipv4Address dip, uint16_t sport, uint16_t dport, uint32_t win, uint64_t baseRtt, Callback<void> notifyAppFinish, Callback<void> notifyAppSent, Callback<void, void *> notifyPacketUpdate){
	// create qp
	...
	// ---- Jalil ----
	qp->SetPacketUpdateCallback (notifyPacketUpdate);
	// ---- Jalil ----
	// add qp
	...
	*/
	...
	if (x == 1 || x == 2){ //generate ACK or NACK
		...
		AddHeader(newp, 0x800);	// Attach PPP header
	        ...
	        // ---- Jalil
		// ensure new packet has a tracer associated with it
	        newp->setPacketTracer (p->getPacketTracer ());
	        // ---- Jalil
...
Ptr<Packet> RdmaHw::GetNxtPacket(Ptr<RdmaQueuePair> qp){
	...
	p->AddHeader (ppp);
	// ---- Jalil
	p->setPacketTracer (qp->m_notifyPacketUpdate);
	// ---- Jalil
