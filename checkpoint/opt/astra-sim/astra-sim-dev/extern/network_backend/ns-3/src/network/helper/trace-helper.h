...
class AsciiTraceHelper
{
  public:
    ...
    // --- Jalil
    template <typename T>
    void HookDefaultPhyTxBeginSinkWithoutContext(Ptr<T> object,
                                                 std::string tracename,
                                                 Ptr<OutputStreamWrapper> file);

    template <typename T>
    void HookDefaultPhyTxEndSinkWithoutContext(Ptr<T> object,
                                               std::string tracename,
                                               Ptr<OutputStreamWrapper> file);

    template <typename T>
    void HookDefaultPhyRxEndSinkWithoutContext(Ptr<T> object,
                                               std::string tracename,
                                               Ptr<OutputStreamWrapper> file);

    static void DefaultPhyTxBeginSinkWithoutContext(Ptr<OutputStreamWrapper> stream,
                                             Ptr<const Packet> p);

    static void DefaultPhyTxEndSinkWithoutContext(Ptr<OutputStreamWrapper> stream,
                                           Ptr<const Packet> p);

    static void DefaultPhyRxEndSinkWithoutContext(Ptr<OutputStreamWrapper> stream,
                                             Ptr<const Packet> p);
    // --- Jalil
    /**
...
template <typename T>
void
AsciiTraceHelper::HookDefaultPhyTxBeginSinkWithoutContext(Ptr<T> object,
                                                       std::string tracename,
                                                       Ptr<OutputStreamWrapper> file)
{
    bool result = object->TraceConnectWithoutContext(
        tracename,
        MakeBoundCallback(&DefaultPhyTxBeginSinkWithoutContext, file));
    NS_ASSERT_MSG(result == true,
                  "AsciiTraceHelper::HookDefaultPhyTxBeginSinkWithoutContext():  Unable to hook \""
                      << tracename << "\"");
}

template <typename T>
void
AsciiTraceHelper::HookDefaultPhyTxEndSinkWithoutContext(Ptr<T> object,
                                                       std::string tracename,
                                                       Ptr<OutputStreamWrapper> file)
{
    bool result = object->TraceConnectWithoutContext(
        tracename,
        MakeBoundCallback(&DefaultPhyTxEndSinkWithoutContext, file));
    NS_ASSERT_MSG(result == true,
                  "AsciiTraceHelper::HookDefaultPhyTxEndSinkWithoutContext():  Unable to hook \""
                      << tracename << "\"");
}

template <typename T>
void
AsciiTraceHelper::HookDefaultPhyRxEndSinkWithoutContext(Ptr<T> object,
                                                       std::string tracename,
                                                       Ptr<OutputStreamWrapper> file)
{
    bool result = object->TraceConnectWithoutContext(
        tracename,
        MakeBoundCallback(&DefaultPhyRxEndSinkWithoutContext, file));
    NS_ASSERT_MSG(result == true,
                  "AsciiTraceHelper::HookDefaultPhyRxEndSinkWithoutContext():  Unable to hook \""
                      << tracename << "\"");
}
