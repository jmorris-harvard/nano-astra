...
// ---- Jalil ----
void RdmaClient::PacketUpdate (void *input) {
  this->msg_handler (input);
}

// void RdmaClient::SetFn(void (*msg_handler)(void *fun_arg), void *fun_arg) {
void RdmaClient::SetFn(std::function<void(void *)> &msg_handler, void *fun_arg) {
// ---- Jalil ----
...
void RdmaClient::StartApplication(void) {
  ...
  rdma->AddQueuePair(src, dest, tag, m_size, m_pg, m_sip, m_dip, m_sport,
		     ...
		     MakeCallback(&RdmaClient::PacketUpdate, this));
  // ---- Jalil ----
}
