...
{
    NS_LOG_FUNCTION(stream << p);
    *stream->GetStream() << "RxBegin " << Simulator::Now().GetSeconds() << " " << p->GetUid () << " " << p->GetSize () << std::endl;
}

void
AsciiTraceHelper::DefaultPhyTxBeginSinkWithoutContext(Ptr<OutputStreamWrapper> stream,
                                                   Ptr<const Packet> p)
{
    NS_LOG_FUNCTION(stream << p);
    *stream->GetStream() << "TxBegin " << Simulator::Now().GetSeconds() << " " << p->GetUid () << " " << p->GetSize () << std::endl;
}

void
AsciiTraceHelper::DefaultPhyTxEndSinkWithoutContext(Ptr<OutputStreamWrapper> stream,
                                                   Ptr<const Packet> p)
{
    NS_LOG_FUNCTION(stream << p);
    *stream->GetStream() << "TxEnd " << Simulator::Now().GetSeconds() << " " << p->GetUid () << " " << p->GetSize () << std::endl;
}

void
AsciiTraceHelper::DefaultPhyRxEndSinkWithoutContext(Ptr<OutputStreamWrapper> stream,
                                                   Ptr<const Packet> p)
{
    NS_LOG_FUNCTION(stream << p);
    *stream->GetStream() << "RxEnd " << Simulator::Now().GetSeconds() << " " << p->GetUid () << " " << p->GetSize () << std::endl;
}
// ---- Jalil
