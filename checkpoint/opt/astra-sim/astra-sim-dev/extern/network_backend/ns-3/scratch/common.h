...
uint64_t ComputeCost (std::vector<Ptr<Node>> path) {
  ...
  while (it != path.end ()) {
    // get next link
    dst = *it;
    // add cost (assumes all links from src to dst have the same cost) TODO
    auto spec = nbr2if[src][dst][0];
...
void GetRoute (Ptr<Node> host) {
  ...
  p.push_back (host);
  // assumes all nodes links from a to b have the same cost
...
void CalculateRoute(Ptr<Node> host) {
  ...
  for (int i = 0; i < (int)q.size(); i++) {
	...
	// Jalil - add this delay
        delay[next] = delay[now] + it->second[0].delay;
        txDelay[next] = txDelay[now] +
                        packet_payload_size * 1000000000lu * 8 / it->second[0].bw;
        bw[next] = std::min(bw[now], it->second[0].bw);
  ...
  for (auto i = nextHop.begin(); i != nextHop.end(); i++) {
    ...
    for (auto j = table.begin(); j != table.end(); j++) {
      ...
      for (int k = 0; k < (int)nexts.size(); k++) {
        ...
        // change to vector interface
        for (auto it = nbr2if[node][next].begin (); it != nbr2if[node][next].end (); it++) { 
          uint32_t interface = it->idx;
...
void TakeDownLink(NodeContainer n, Ptr<Node> a, Ptr<Node> b) {
  ...
  // -- Jalil
  for (auto it = nbr2if[a][b].begin (); it != nbr2if[a][b].end (); it++) {
    it->up = false;
  }
  for (auto it = nbr2if[b][a].begin (); it != nbr2if[b][a].end (); it++) {
    it->up = false;
  }
  // -- Jalil
  nextHop.clear();
  ...
  for (auto it = nbr2if[a][b].begin (); it != nbr2if[a][b].end (); it++) {
    DynamicCast<QbbNetDevice>(a->GetDevice (it->idx))->TakeDown();
  }
  for (auto it = nbr2if[b][a].begin (); it != nbr2if[b][a].end (); it++) {
    DynamicCast<QbbNetDevice>(b->GetDevice (it->idx))->TakeDown();
  }
...
bool SetupNetwork(void (*qp_finish)(FILE *, Ptr<RdmaQueuePair>)) {
  ...
  // -- Jalil --
  std::cout << "reading in links" << std::endl;
  // -- Jalil --
  for (uint32_t i = 0; i < link_num; i++) {
    ...
    Interface intfs, intfd;
    intfs.idx = DynamicCast<QbbNetDevice>(d.Get(0))->GetIfIndex();
    intfs.up = true;
    intfs.delay = DynamicCast<QbbChannel>(DynamicCast<QbbNetDevice>(d.Get(0))->GetChannel())
            ->GetDelay()
            .GetTimeStep();
    intfs.bw = DynamicCast<QbbNetDevice>(d.Get(0))->GetDataRate().GetBitRate();
    intfs.err = error_rate;
    nbr2if[snode][dnode].push_back (intfs);

    intfd.idx = DynamicCast<QbbNetDevice>(d.Get(1))->GetIfIndex();
    intfd.up = true;
    intfd.delay = DynamicCast<QbbChannel>(DynamicCast<QbbNetDevice>(d.Get(1))->GetChannel())
            ->GetDelay()
            .GetTimeStep();
    intfd.bw = DynamicCast<QbbNetDevice>(d.Get(1))->GetDataRate().GetBitRate();
    intfd.err = error_rate;
    nbr2if[dnode][snode].push_back (intfd);
  ...
  for (auto iti = nbr2if.begin (); iti != nbr2if.end (); iti++) {
    ...
    auto dst = iti->second;
    std::cout << "\t" << host->GetId () << std::endl;
    for (auto itj = dst.begin (); itj != dst.end (); itj++) {
      auto dstnode = itj->first;
      auto connections = itj->second;
      for (auto itk = connections.begin (); itk != connections.end (); itk++) {
        auto link = *itk;
        std::cout << "\t\tlink (" << link.idx << ") --> " << dstnode->GetId () << std::endl;
      }
    }
  }
  /*
  std::cout << std::endl;
  std::cout << "Hit CTRL-C" << std::endl;
  while (1);
  */
  // ----- Jalil -----
  ...
  // dump link speed to trace file
  {
    ...
    for (auto i : nbr2if) {
      for (auto j : i.second) {
        ...
        // -- Jalil - changed link nbr2if to vector
        for (auto k : j.second) {
          uint8_t intf = k.idx;
          uint64_t bps =
              DynamicCast<QbbNetDevice> (i.first->GetDevice (intf))
                  ->GetDataRate ()
                  .GetBitRate ();
