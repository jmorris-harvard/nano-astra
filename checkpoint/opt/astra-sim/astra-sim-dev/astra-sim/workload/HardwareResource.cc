...
// ---- Jalil
// ---- added max gpu comm ops 
HardwareResource::HardwareResource(uint32_t num_npus, uint32_t max_gpu_comm_ops)
    : num_npus(num_npus),
      _max_gpu_comm_ops (max_gpu_comm_ops),
...
bool HardwareResource::is_available(
    ...
    if (node->is_cpu_op()) {
        ...
        if (node->type() == ChakraNodeType::COMP_NODE) {
            ...
            if (num_in_flight_gpu_comm_ops < this->_max_gpu_comm_ops /* Jalil changed maximum comm ops to 2 */) {
                return true;
            } else {
                if (node->type() == ChakraNodeType::COMM_RECV_NODE) {
                    return true;
