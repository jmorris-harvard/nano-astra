...
Sys::Sys(int id,
    ...
    // ---- Jalil Morris
    // moved this here to avoid segmentation
    this->total_nodes = 1;
    // this too
...
bool Sys::initialize_sys(string name) {
    ...
    if (j.contains ("statistics-enabled")) {
      // std::cout << "stats enabled" << std::endl;
      if (j["statistics-enabled"] != 0) {
        if (!j.contains ("statistics-config")) {
          std::cerr << "no statistic configuration given" << std::endl;
	  exit (EXIT_FAILURE);
        }
        this->workload->addStats (j["statistics-config"]);
      }
    }
    // --- Jalil Morris
...
void Sys::handleEvent(void* arg) {
	...
	// ---- Jalil ----
	if (event == EventType::CompFinished) {
